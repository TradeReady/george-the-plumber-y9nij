...
export default function Template1Site({ site, imagePack }) {
  ...
  // Hero image resolution — mirrors SiteHero logic
  const getHeroImage = () => {
  ...
  useEffect(() => {
    ...
    s2.innerHTML=`window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','${measurementId}',{page_title:document.title,page_location:window.location.href,send_page_view:true});`;
  ...
  return (
    <div style={{ fontFamily: fontStyle, backgroundColor: bg, color: textColor }}>
        ...
        <div style={{ position:'relative', zIndex:10, maxWidth:1152, margin:'0 auto', padding: isMobile ? '100px 20px 60px' : '128px 24px 80px', width:'100%' }}>
          <div style={{ maxWidth:640 }}>
            ...
            <motion.p initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.6, delay:0.2 }}
              style={{ fontSize:'1.125rem', color:'rgba(255,255,255,0.75)', marginBottom:16, lineHeight:1.6 }}>
              {gc.subheadline}
            </motion.p>
            {gc.rating && (
              <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.3 }} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:32 }}>
            ...
            <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.35 }} style={{ display:'flex', flexWrap:'wrap', gap:10, marginBottom:32 }}>
              {['Fully Licensed','Insured','Local Experts'].map(badge => (
                <span key={badge} style={{ display:'inline-flex', alignItems:'center', gap:6, backgroundColor:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)', color:'#fff', fontSize:'0.75rem', padding:'6px 14px', borderRadius:9999 }}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                  {badge}
                </span>
              ))}
            </motion.div>
            <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.4 }} style={{ display:'flex', flexDirection: isMobile ? 'column' : 'row', gap:12, width: isMobile ? '100%' : 'auto' }}>
              {gc.phone && (
                <a href={`tel:${gc.phone}`} style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', gap:8, padding:'14px 24px', color:'#fff', fontWeight:600, fontSize:'0.875rem', border:'1px solid rgba(255,255,255,0.3)', borderRadius:buttonRadius, textDecoration:'none', backdropFilter:'blur(4px)', width: isMobile ? '100%' : 'auto' }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.41 2 2 0 0 1 3.6 1.21h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 8.7a16 16 0 0 0 6 6l.86-.86a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 21.5 16z"/></svg>
                  Call Now
                </a>
              )}
              <a href="#contact" style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', gap:8, padding:'14px 24px', backgroundColor:primary, color:'#fff', fontWeight:600, fontSize:'0.875rem', borderRadius:buttonRadius, textDecoration:'none', width: isMobile ? '100%' : 'auto' }}>
        ...
        <div style={{ maxWidth:1152, margin:'0 auto', padding:'0 24px', display:'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: isMobile ? 16 : 24 }}>
          {statsData.map((s,i) => (
            <motion.div key={i} initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.7, delay:i*0.12, ease:[0.2,0.8,0.2,1] }} style={{ textAlign:'center' }}>
              ...
              <div style={{ fontSize:'0.875rem', color:'#6b7280', textTransform:'uppercase', letterSpacing:'0.05em' }}>{s.label}</div>
            </motion.div>
          ))}
        </div>
      </section>
          ...
          <div style={{ maxWidth:1152, margin:'0 auto', padding:'0 24px' }}>
            ...
            {services.length > 3 && (
              <div style={{ textAlign:'center', marginTop:40 }}>
                <button
                  onClick={() => { setShowAllServices(v => !v); if(window.gtag)window.gtag('event','view_all_services',{event_category:'engagement',event_label:'services_expand',business_name:businessName}); }}
          ...
          <div style={{ maxWidth:1152, margin:'0 auto', padding:'0 24px', display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 32 : 48, alignItems:'center' }}>
            ...
            <motion.div initial={{ opacity:0, x:30 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }}
              ...
              {[
                ...
                <div key={i} style={{ borderRadius:12, padding: isMobile ? 16 : 24, textAlign:'center',
                  backgroundColor: i%2===0 ? primary : `${primary}15`,
                  color: i%2===0 ? '#fff' : textColor }}>
      ...
      {/* ── CTA BANNER ── */}
      <section style={{ position:'relative', overflow:'hidden', padding:'96px 0', backgroundColor:textColor }}>
        ...
        <div style={{ position:'relative', zIndex:10, maxWidth:960, margin:'0 auto', padding:'0 24px', display:'flex', flexWrap:'wrap', alignItems:'center', gap:48 }}>
          <motion.div initial={{ opacity:0, x:-30 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }} style={{ flex:1, minWidth:280 }}>
            ...
            <h2 style={{ fontSize:'clamp(2rem,4vw,3rem)', fontWeight:800, color:'#fff', lineHeight:1.1, marginBottom:16 }}>
          ...
          <motion.div initial={{ opacity:0, x:30 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }} style={{ display:'flex', flexDirection:'column', gap:16, minWidth: isMobile ? '100%' : 260 }}>
            {gc.phone && (
              <a href={`tel:${gc.phone}`} style={{ display:'flex', alignItems:'center', gap:16, padding:20, backgroundColor:primary, borderRadius:16, textDecoration:'none', transition:'transform 0.2s' }}
      ...
      <section id="contact" style={{ padding:'80px 0', backgroundColor:'#fff', overflow:'hidden' }}>
        <div style={{ maxWidth:1152, margin:'0 auto', padding:'0 16px', display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 32 : 48, alignItems:'start' }}>
          <motion.div initial={{ opacity:0, x:-20 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              {[gc.phone && { icon:'phone', label:'Phone', val:gc.phone, href:`tel:${gc.phone}` }, gc.email && { icon:'mail', label:'Email', val:gc.email, href:`mailto:${gc.email}` }, gc.address && { icon:'pin', label:'Address', val:gc.address, href:null }].filter(Boolean).map((c,i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:16, padding:16, backgroundColor:'#fff', borderRadius:12, border:'1px solid #f3f4f6', boxShadow:'0 1px 3px rgba(0,0,0,0.05)' }}>
                  ...
                  <div>
                    ...
                    {c.href ? <a href={c.href} style={{ fontSize:'0.9rem', fontWeight:600, color:textColor, textDecoration:'none' }}>{c.val}</a>
